...
def compute_B_scaled(model, coords_norm, inv_scale):
    """计算原始尺度下的磁场 B = ∇ × A"""
    coords_norm = coords_norm.detach().clone().requires_grad_(True)

    A = model(coords_norm)

    inv_sx, inv_sy, inv_sz = inv_scale[0], inv_scale[1], inv_scale[2]

    grad_outputs = torch.ones_like(A[:, 0:1])

    dAx_dy = torch.autograd.grad(A[:, 0:1], coords_norm, grad_outputs=grad_outputs,
                                 create_graph=True, retain_graph=True, allow_unused=True)[0]
    dAx_dz = torch.autograd.grad(A[:, 0:1], coords_norm, grad_outputs=grad_outputs,
                                 create_graph=True, retain_graph=True, allow_unused=True)[0]
    dAy_dx = torch.autograd.grad(A[:, 1:2], coords_norm, grad_outputs=torch.ones_like(A[:, 1:2]),
                                 create_graph=True, retain_graph=True, allow_unused=True)[0]
    dAy_dz = torch.autograd.grad(A[:, 1:2], coords_norm, grad_outputs=torch.ones_like(A[:, 1:2]),
                                 create_graph=True, retain_graph=True, allow_unused=True)[0]
    dAz_dx = torch.autograd.grad(A[:, 2:3], coords_norm, grad_outputs=torch.ones_like(A[:, 2:3]),
                                 create_graph=True, retain_graph=True, allow_unused=True)[0]
    dAz_dy = torch.autograd.grad(A[:, 2:3], coords_norm, grad_outputs=torch.ones_like(A[:, 2:3]),
                                 create_graph=True, retain_graph=True, allow_unused=True)[0]

    def replace_none(grad, like):
        if grad is None:
            return torch.zeros_like(like, requires_grad=False)
        return grad

    dAx_dy = replace_none(dAx_dy, coords_norm)[:, 1:2]
    dAx_dz = replace_none(dAx_dz, coords_norm)[:, 2:3]
    dAy_dx = replace_none(dAy_dx, coords_norm)[:, 0:1]
    dAy_dz = replace_none(dAy_dz, coords_norm)[:, 2:3]
    dAz_dx = replace_none(dAz_dx, coords_norm)[:, 0:1]
    dAz_dy = replace_none(dAz_dy, coords_norm)[:, 1:2]

    Bx = dAz_dy * inv_sy - dAy_dz * inv_sz
    By = dAx_dz * inv_sz - dAz_dx * inv_sx
    Bz = dAy_dx * inv_sx - dAx_dy * inv_sy
...
def predict_dT(model, coords_norm, inv_scale, alpha, beta, gamma, batch_size=4096):
    ...
    for i in range(0, len(coords_norm), batch_size):
        coords_batch = torch.tensor(coords_norm[i:i + batch_size], dtype=torch.float32, device=device)
...
def main():
    ...
    model_path = 'vector_potential_pinn_final.pth'
    norm_params_path = 'normalization_params.npz'
    I_deg = 43.1
    D_deg = -2.0
